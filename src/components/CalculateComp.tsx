...
const findPerimeter = ( length:number, width:number):void => {
  ...
  return (
    ...
    <div className="flex flex-col gap-4 w-70 ">
      <input onChange={(e)=> setLength(Number(e.target.value))} type="text" value={Length} placeholder="Enter your length" />
      <input onChange={(e)=> setWidth(Number(e.target.value))} type="text" value={Width} placeholder="Enter Your Width" />
             <button className="bg-teal-600">Calculate Area</button>
            <button className="bg-teal-600 ">Calculate Parameter</button>
</div>
